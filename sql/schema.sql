...
CREATE TABLE emart_compatibility (
    stock_number              VARCHAR2(7),
    compatible_manufacturer   VARCHAR2(50),
    compatible_model_number   VARCHAR2(50),
...
CREATE TABLE emart_orders (
    ...
    CONSTRAINT chk_order_money CHECK (
        subtotal >= 0 AND discount >= 0 AND shipping_fee >= 0 AND total >= 0
    )
);
...
CREATE TABLE edepot_inventory (
    ...
    CONSTRAINT chk_inventory_quantities CHECK (
        quantity >= 0 AND minimum_stock >= 0 AND maximum_stock >= 0 AND replenishment >= 0
    ),
...
CREATE TABLE edepot_shipping_notices (
    ...
    shipping_company  VARCHAR2(50) NOT NULL,
